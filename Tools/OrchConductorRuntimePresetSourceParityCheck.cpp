                               ...
                               int reservedCc)
{
    ...
    if (ok)
        std::cout << "[PASS] " << domainName << " runtime boundary parity checks" << std::endl;
                             ...
                             int reservedCc)
{
    ...
    if (ok)
        std::cout << "[PASS] combi runtime boundary parity checks" << std::endl;
...
bool verifyRuntimeBoundaryLoadedLibrary(const orchconductor::PresetLibraryDefinition& library)
{
    bool ok = true;
    ...
    ok = check(library.isValid(), "Runtime boundary library failed final isValid() check.") && ok;
...
int main()
{
    std::cout << "OrchConductor runtime JSON boundary parity check" << std::endl;
    std::cout << "------------------------------------------------" << std::endl;
    ...
    ok = checkPass(result.wasLoaded(), "runtime boundary loaded embedded JSON when feature gate is ON") && ok;
    ok = checkPass(! result.requiresHardcodedFallback(), "runtime boundary does not require fallback after ON load") && ok;
    ok = checkPass(result.diagnosticMessage.isNotEmpty(), "runtime boundary diagnostic message is present") && ok;

    if (result.wasLoaded())
        ok = verifyRuntimeBoundaryLoadedLibrary(result.library) && ok;

    if (! ok)
        return fail("Runtime JSON boundary ON parity verification failed.");

    std::cout << "------------------------------------------------" << std::endl;
    std::cout << "[PASS] Runtime JSON boundary ON parity verification completed successfully." << std::endl;
#else
    bool ok = true;

    ok = checkPass(! result.wasLoaded(), "runtime boundary does not load when feature gate is OFF") && ok;
    ok = checkPass(result.requiresHardcodedFallback(), "runtime boundary requires hardcoded fallback when feature gate is OFF") && ok;
    ok = checkPass(result.diagnosticMessage.isNotEmpty(), "runtime boundary disabled diagnostic message is present") && ok;

    if (! ok)
        return fail("Runtime JSON boundary OFF parity verification failed.");

    std::cout << "------------------------------------------------" << std::endl;
    std::cout << "[PASS] Runtime JSON boundary OFF parity verification completed successfully." << std::endl;
#endif
